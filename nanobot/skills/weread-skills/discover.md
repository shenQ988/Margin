...
- 使用 `recommend_author` 时：只展示该动作返回的 `candidates`；`validation.rejectedCandidates` 是已验证的禁推书（包括已拥有作品的双语、翻译或不同版本），不得展示。若其 `profile` 中有深读主题、深读书或在读书，必须用其中一个真实信号给出一句简短的个性化理由，不得编造用户偏好或书籍内容。
- 使用 `advisor` 时：先遵循返回的 `workflow.checkpoint` 确认数量、来源、语言和篇幅约束；约束已知后只展示 `candidates`，并按 `workflow.route` 选择阅读顾问建议或入门路径。
