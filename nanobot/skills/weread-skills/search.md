...
- 用户说"搜一下 xx 作者""查作者 xx"，且目的是找作者资料/作者主页 → `scope=6`
- 用户说"推荐/找/看看 xx 的书""xx 还有什么书"，即使关键词是作者名，仍是找书 → `scope=10`；不要用 `scope=6`
...
- 推荐某位作者的更多书时，优先使用 `recommend_author`；它会以 `scope=10` 搜电子书，并基于实时书架和笔记排除已拥有的书与重复版本。回复只能推荐其返回的候选书目。
