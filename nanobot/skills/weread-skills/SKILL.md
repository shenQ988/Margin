---
name: weread-skills
description: 微信读书助手 — 搜索书籍、管理书架、查看笔记划线、浏览书评、阅读统计、发现推荐好书。调用方式：使用 weread 原生工具（action 参数指定能力），不需要 curl 或安装任何 CLI App；调用前建议先用 read_file 读取本 SKILL.md 了解字段含义和展示规则。
version: 1.0.4
metadata: {"nanobot":{"emoji":"📖","requires":{"env":["WEREAD_API_KEY"]}}}
---

# WeRead — 微信读书助手

通过原生 `weread` 工具调用微信读书接口，提供搜索、书架、笔记、书评等能力。

**重要：调用能力时直接使用 `weread` 工具（`action` 参数指定具体能力，见下方「接口调用规范」的映射表），不需要拼接 curl 命令，也不存在需要单独安装的 CLI App。工具已自动处理鉴权和版本上报。本文件是自包含参考文档，所有字段含义、计数口径和展示规则均已内嵌在下方对应章节中，调用前建议先阅读，避免凭字段名或经验猜测含义。**

## 支持的能力

| 能力 | 说明 | 用户示例 | 详细说明章节 |
|------|------|----------|----------|
| 搜索书籍 | 在书城搜索 | "帮我搜一下三体" | 「搜索」 |
| 书籍信息 | 查看书籍详情、章节目录、阅读进度 | "这本书有多少章" "我读到哪了" | 「书籍信息与阅读进度」 |
| 书架管理 | 查看书架 | "看看我的书架" | 「书架管理」 |
| 阅读统计 | 阅读时长、天数、偏好分析、阅读统计摘要 | "我这个月读了多久" "今年读了几本书" | 「阅读统计」 |
| 笔记划线 | 查看个人笔记数量与内容，包括划线、想法/点评、书签数量 | "看看我在三体里的笔记" "导出我的划线" "在这本书有多少笔记" | 「笔记/划线」 |
| 章节热门划线 | 查看书籍/章节热门划线、划线热度及划线下想法 | "看看这章有什么热门划线" "这段话下面有什么想法" | 「笔记/划线」 |
| 书籍点评 | 查看书籍的公开点评 | "三体这本书有什么点评？" "看看推荐的点评" | 「书籍点评」 |
| 推荐好书 | 个性化推荐/相似推荐 | "给我推荐几本书" | 「发现推荐好书」 |
| 综合阅读概况 | 书架 + 最近进度 + 笔记概况一次性汇总 | "我的阅读概况怎么样" | 「综合阅读概况（书架 + 进度 + 笔记）」 |

根据用户意图，直接参考本文件下方对应章节了解接口参数、回包结构和工作流；无需读取任何外部文件。
...
使用 `weread` 工具，通过 `action` 参数选择具体能力；工具在内部自动处理 Agent API Gateway 的鉴权（`WEREAD_API_KEY`）和版本上报（`skill_version`），无需手动拼接。

**action 与接口映射：**

| action | 对应接口 | 必填参数 | 说明 |
|--------|----------|----------|------|
| `search` | `/store/search` | `keyword` | 可选 `scope`（见「搜索」章节）、`count`、`max_idx` |
| `shelf` | `/shelf/sync` | 无 | |
| `notebooks` | `/user/notebooks` | 无 | 可选 `count`、`last_sort`（翻页游标） |
| `bookmarklist` | `/book/bookmarklist` | `book_id` | |
| `reviews_mine` | `/review/list/mine` | `book_id` | 可选 `synckey`、`count` |
...
```json
{"action": "search", "keyword": "三体", "count": 10}
```

```json
{"action": "notebooks", "count": 100, "last_sort": 1516907353}
```

### 响应格式

- 工具返回原始 API JSON（已裁剪字段，只含核心字段），需按本文件下方对应章节的字段说明解读
- 无结果或调用失败时，工具会返回 `Error: ...` 文本，据此提示用户，不要编造数据

### 通用规则

1. **能力文档预检**：调用任何 action 前，必须先根据「支持的能力」表阅读本文件下方对应章节，确认字段含义、单位、计数口径和工作流；禁止仅凭字段名或经验猜测含义。
2. **字段解释优先级**：解释返回结果时，以本文件下方对应章节中的字段说明为准；如果回包字段名和直觉含义冲突，必须服从章节说明，不得直接翻译字段名。
3. **bookId 解析**：用户输入书名时，先用 `action=search` 获取 bookId，再执行后续操作
4. **书架数量**：`action=shelf` 回答"书架有多少本书/多少条目"时，必须按 `books.length + albums.length + (mp 非空 ? 1 : 0)` 计算；`albums[]` 是专辑/有声书，也属于书架里的书，详细规则见「书架管理」章节
5. **结果展示**：列表用编号展示方便选择；搜索结果重点展示书名、作者、评分；展示回包信息时，字段**禁止**直接翻译，应该参考对应章节中的说明内容提供
6. **上下文衔接**：对话中记住已查询的 bookId，后续操作无需用户重复提供
7. **深度链接**：优先使用回包中的 `deepLink` 字段作为跳转链接，展示为 `[打开阅读]({deepLink})`；若回包没有 `deepLink`，不要自行拼接 `weread://` 链接
8. **数据展示规范**：
...
书籍的公开点评（区别于个人笔记/划线，个人笔记见「笔记/划线」章节）。

### 核心约束

1. 所有回复必须基于接口返回数据，不编造、不推断。
2. 公开点评不属于个人笔记，是其他读者对书籍的评价；个人笔记/划线见「笔记/划线」章节。
...
#### 2. 获取阅读进度
从 `books[]` 中按 `readUpdateTime` 降序取最近阅读的前 5 本书，分别调 `/book/getprogress` 获取进度和累计阅读时长，具体见「书籍信息与阅读进度」章节。
...
如果用户要查看某本书的具体笔记内容，再按「笔记/划线」章节调 `/book/bookmarklist` 和 `/review/list/mine`。
